...
string cmD (string crs, char t) //Creates a schedule (by days); must fix so it also gives times.
{
 /*   string crs; //course name
    ...
    char s = 0;
    system("cls");
    cout << "\nThe following questions involve the scheduled course meetings. To skip this step press ESC or to continue, press ENTER. ";
    s = _getch();
    if (s != 27 && s == 13)
    {
 ...
 (Co/Pre)requisite counter), and y (the course counter). It returns a string that is acquired by Rsk().*/
{
    ...
    while (s != 78 && k <= r)
    {
        ...
        cin >> z;
        b = "\t-->" + wrts(t,a,k,z,y) + "Credit hours: " + its(z) + " |*| ";
        if (seek(N,a)!=true) //if the course isn't already in the file, then it'll provide it with a unique ID.
        {
            b = "\t-->" + wrts(t,a,k,z,y) +" CID: " + cmD(a,t);
        }
        if (k > r)
        {
            cout << "You have exceeded the number of co- and/or prerequisites you can add. " << endl;
            s = 27;
            return b;
        }
        if (s == 27)
...
It uses Rsk() to provide the string MReqs () and GEReqs() write into the files they create. */
{
 ...
 //   cout << "\nIf this is a prerequisite for " << a <<" press P. Or press ESC to abort.";
    while (s != 27) //start loop
    {
        ...
        case 13: // ENTER
            {
                cout << endl << "Please enter co/prerequisite course name. ";
                cin >> a;
                b = RSk(N,t,a,y);
                return b + "\n";
            } //end case 78
        ...
        default:
            {
                cout << "If " << a << " has a co/prerequisite, press ENTER or press ESC to exit." << endl;
                s = _getch();
            }
        }
    }
        ...
        case 89:
        {
            ...
            /// THE CO/PREREQUISITE LOOP STARTS
                    if (_getch()!= 27 && n <= v)
                    {
                        k++;
                        n++;
                        w = PReqs(N,t,a,y); //NOTES: FIND SOURCE OF ERROR & FIX ASAP
                    } else {k = 0;}
                    y++;
                    n++;
                    }
    ...
    while(s != 27)
    {
        switch(s)
        {
            ...
            case 13: //ENTER key
            {
                ...
                cin >> creds;
                z += creds;
                cout << "There are " << z << " credits required for " << maj << ". Correct? Y/N ";
                cin >> nr;
                while (nr != "Y" && nr == "N")
                {
                    cout << "What is your major? ";
                    ...
                    {
                        cout << "Please answer the following question using Y or N only." << endl;
                        cout << "There are " << z << " credits required for " << maj << ". Correct? Y/N";
                        cin >> nr;
                    }
                }
